...
std::mutex ProxyIDirectInputDevice8A::m_Mutex;
...
HRESULT WINAPI hk_DirectInput8Create(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf, LPVOID *ppvOut, IUnknown *punkOuter)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		CreateDevice = DetourVTable(*(uint8_t **)(*ppvOut), (uint8_t*)&hk_CreateDevice, 3);
	}
...
{
	// Allow acquire if we can do input (Prevent malicious calls)
	if (m_EnableInput)
	{
...
void ProxyIDirectInputDevice8A::PatchDInput()
{
	LPVOID OldAddress = 0;

	// Patch DInput so we can manipulate the focus events
	DetourIAT(GetModuleHandle(NULL), "DirectInput8Create", &OldAddress, (LPVOID)&hk_DirectInput8Create);
}